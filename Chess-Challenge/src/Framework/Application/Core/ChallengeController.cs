...
namespace ChessChallenge.Application
{
    public class ChallengeController
    {
        ...
        public ChallengeController()
        {
            ...
            BotStatsA = new BotMatchStats("Evilbot");
            BotStatsB = new BotMatchStats("StupidFish");
            botMatchStartFens = FileHelper.ReadResourceFile("Fens.txt").Split('\n').Where(fen => fen.Length > 0).ToArray();
            botTaskWaitHandle = new AutoResetEvent(false);

            StartNewGame(PlayerType.MyBot, PlayerType.EvilBot); //this where the default game starts
        }
        ...
        ChessPlayer CreatePlayer(PlayerType type)
        {
            return type switch
            {
                ...
                PlayerType.EvilBot => new ChessPlayer(new V6_bot(), type, GameDurationMilliseconds),
